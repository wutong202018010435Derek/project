...
public class SysControlController extends BaseController {
    ...
    private String prefix = "system/control/device";
    ...
    public AjaxResult editSave(@Validated SysDictType dict) {
        System.out.println(dict.getHum() + "  " + dict.getTemp() + "  " + dict.getCarbon() + "  " + dict.getLight());
        SysDictType md = dictTypeService.selectDictTypeById(dict.getDictId());
        if (md != null) {
            dict.setDictName(md.getDictName());
            dict.setDictMqtt(md.getDictMqtt());
        }
        return null;
//        return toAjax(mqttConsumerClient.pushMsg(MqttConsumerClient.changeToSendData(2, dict)));
